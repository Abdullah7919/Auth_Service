...
class UserService{
    ...
    async isAuthenticated(token){
        try {
            ...
            const response=await this.userRepository.getById(user.id);
            if(!response){
                throw {error:"User not exist for corresponding token"};
            }
            return response.id;
        } catch (error) {
            console.log("Somethin went wrong in auth process");
            throw error;
        }
    }
